...
public abstract class Table<T extends PDPage> {
	...
	protected float verticalCellMargin = 2f;
	protected float horizontalCellMargin = 2f;
	...
	public Table(float yStart, float yStartNewPage, float bottomMargin, float width, float margin,
			PDDocument document, T currentPage, boolean drawLines, boolean drawContent)
			throws IOException {
	...
	public Table(float yStartNewPage, float bottomMargin, float width, float margin,
			PDDocument document, boolean drawLines, boolean drawContent) throws IOException {
		this.document = document;
		this.drawLines = drawLines;
		this.drawContent = drawContent;
		// Initialize table
	...
	public void drawTitle(String title, PDFont font, int fontSize, TextType textType)
			throws IOException {
	...
	private void drawCellContent(Row row) throws IOException {
		...
		while (cellIterator.hasNext()) {
			...
			for (String line : cell.getParagraph().getLines()) {
				...
				if (numLines > 0)
					this.tableContentStream.appendRawCommands("T*\n");
				numLines--;
			}
	...
	private void drawLine(String type, float xStart, float yStart, float xEnd, float yEnd)
			throws IOException {
	...
	private void endTable() throws IOException {
		...
		if (drawLines) {
			// Draw line at bottom
			drawLine("Row Bottom Border ", this.margin, this.yStart, this.margin + width + xSpacing,
					this.yStart);
		}
	...
	public boolean isEndOfPage(Row row) {
		...
		// If we are closer than 75 from bottom of currentPage, consider this the
		// end of the currentPage
